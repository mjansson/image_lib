...
main_initialize(void) {
	...
	application.name = string_const(STRING_CONST("Image library test suite"));
	...
	return image_module_initialize(image_config);
}
...
main_run(void* main_arg) {
	...
	log_infof(HASH_TEST, STRING_CONST("Image library v%s built for %s using %s (%s)"),
	          string_from_version_static(image_module_version()).str, FOUNDATION_PLATFORM_DESCRIPTION,
	...
	test_run_fn tests[] = {test_image_run, 0};
